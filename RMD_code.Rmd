...
Upon investigating the `Age` variable, it is noticable that there is a considerable share of missing values. Before attempting to solve this issue, however, it would be wise to compare the `Age` variable against the newly created `titleGroup` variable because the `titleGroup` of the passenger may very well have a strong link the the `Age`. 
